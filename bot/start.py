...
###############------Add Cover------###############
@TELETHON_CLIENT.on(events.NewMessage(incoming=True, pattern='/addcover', func=lambda e: user_auth_checker(e)))
async def _add_cover(event):
        chat_id = event.message.chat.id
        user_id = event.message.sender.id
        if user_id not in get_data():
                await new_user(user_id, SAVE_TO_DATABASE)
        ...
        if link=="invalid":
            ...
            new_event = await ask_media_OR_url(event, chat_id, user_id, ["/addcover", "stop"], "Send Video or URL", 120, "video/", True)
            if new_event and new_event not in ["cancelled", "stopped"]:
                link = await get_url_from_message(new_event)
            else:
                return

        cover_event = await ask_media_OR_url(event, chat_id, user_id, ["/addcover", "stop"], "Send Cover Image or URL", 120, "image/", True, allow_magnet=False)

        if not cover_event or cover_event in ["cancelled", "stopped"]:
            return

        user_name = get_username(event)
        user_first_name = event.message.sender.first_name

        temp_dir = f"{download_dir}/{user_id}/{gen_random_string(5)}"
        create_direc(temp_dir)
        cover_art_path = None

        if cover_event.message.file:
            cover_art_path = await cover_event.download_media(file=temp_dir)
        else:
            cover_url = str(cover_event.message.message)
            cover_art_path = f"{temp_dir}/cover.jpg"
            try:
                dw_file_from_url(cover_url, cover_art_path)
            except Exception as e:
                await event.reply(f"❗Error downloading cover image: {e}")
                return

        if not exists(cover_art_path):
            await event.reply("❗Failed to get cover image.")
            return

        process_status = ProcessStatus(user_id, chat_id, user_name, user_first_name, event, Names.addcover, custom_file_name, cover_art=cover_art_path)
...
###############------Remove Cover------###############
@TELETHON_CLIENT.on(events.NewMessage(incoming=True, pattern='/removecover', func=lambda e: user_auth_checker(e)))
async def _remove_cover(event):
        chat_id = event.message.chat.id
        user_id = event.message.sender.id
        if user_id not in get_data():
                await new_user(user_id, SAVE_TO_DATABASE)
        ...
        if link=="invalid":
            ...
            new_event = await ask_media_OR_url(event, chat_id, user_id, ["/removecover", "stop"], "Send Video or URL", 120, "video/", True)
            if new_event and new_event not in ["cancelled", "stopped"]:
                link = await get_url_from_message(new_event)
            else:
                return
        ...
        user_name = get_username(event)
        user_first_name = event.message.sender.first_name
        process_status = ProcessStatus(user_id, chat_id, user_name, user_first_name, event, Names.removecover, custom_file_name)
